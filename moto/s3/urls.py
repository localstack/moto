...
url_paths = {
    # subdomain bucket
    "{0}/$": S3Response.bucket_response,
    # subdomain key of path-based bucket
    "{0}/(?P<key_or_bucket_name>[^/]+)$": S3Response.ambiguous_response,
    "{0}/(?P<key_or_bucket_name>[^/]+)/$": S3Response.ambiguous_response,
    # path-based bucket + key
    "{0}/(?P<bucket_name_path>[^/]+)/(?P<key_name>.+)": S3Response.key_response,
    # subdomain bucket + key with empty first part of path
    "{0}/(?P<key_name>/.*)$": S3Response.key_response,
}
